...
BEGIN
  ...
  effective_cooldown := GREATEST(
    COALESCE(p_cooldown, 0),
    CASE
      WHEN p_ad_type = 'interstitial' THEN COALESCE((SELECT value FROM public.app_settings WHERE key='auto_next_min_view_seconds'), '15')::int
      ELSE 0
    END
  );

  EXECUTE format('SELECT ($1).%I', p_last_col) INTO last_ts USING u;
  IF last_ts IS NOT NULL AND EXTRACT(EPOCH FROM (now() - last_ts)) < effective_cooldown THEN
    RAISE EXCEPTION 'Wait % seconds before next ad', CEIL(effective_cooldown - EXTRACT(EPOCH FROM (now() - last_ts)));
  END IF;
